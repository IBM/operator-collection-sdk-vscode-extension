...
import { ResourceTreeItem } from "../../treeViews/resourceItems/resourceTreeItems";
...
describe("Extension Test Suite", async () => {
  ...
  describe("When validating commands", () => {
    it("Should create an operator", async () => {
      try {
        ...
        await helper.pollOperatorInstallStatus(imsOperatorItem.operatorName, 60);
      } catch (e) {
  ...
  describe("When validating the Tree View", () => {
    ...
    it("Should validate the IMS Broker Custom Resources in OpenShift Resources", async () => {
      ...
      imsOperatorCollectionParent = imsBrokerCustomResources.find(brokerResource => brokerResource instanceof OperatorCollectionItem) as OperatorCollectionItem;
      ...
      imsSubOperatorConfigParent = imsBrokerCustomResources.find(brokerResource => brokerResource instanceof SubOperatorConfigItem) as SubOperatorConfigItem;
      ...
      imsCustomResourceParents = imsBrokerCustomResources.filter(brokerResource => brokerResource instanceof CustomResourceItem) as CustomResourceItem[];
      const tmdbKind = "TMDB";
