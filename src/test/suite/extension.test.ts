...
describe("Extension Test Suite", async () => {
  ...
  describe("When validating context-menu functions", async () => {
    ...
    const ocWorkspaceURI = vscode.Uri.file(testVars.fixturePath);
    const customCollectionPath = path.join(ocWorkspaceURI.fsPath, customCollectionName);
    ...
    before(() => {
      ...
      fs.mkdirSync(customCollectionPath, { recursive: true });
    });
    ...
    it("Should the conversion of a collection to an air-gapped collection.", async () => {
      try {
        ...
        const fileContent = `
        ---
          - name: ${requirementNamespace}.${requirementCollection}
            version: "${requirementVersion}"
        `;
        await workspace.createWorkspaceFile(filePath, fileContent);

        if (fs.existsSync(filePath)) {
          await vscode.commands.executeCommand(VSCodeCommands.convertToAirgapCollection, customCollectionURI);
          ...
          assert.fail("Failed to create requirements.yml file when converting collection to an air-gapped collection.");
        }
      } catch (e) {
    ...
    //   try {
    //     vscode.commands.executeCommand(VSCodeCommands.initCollection, ocWorkspaceURI, undefined, initCollectionLogPath);
    //   } catch (e) {
    //     console.log("Printing OC Login logs");
    //     helper.displayCmdOutput(ocLoginLogPath);
