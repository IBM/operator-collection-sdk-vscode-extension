...
export class SubOperatorConfigsItem extends ResourceTreeItem {
  constructor(
    ...
    super(
      subOperatorConfigObj.metadata.name,
...
export async function getSubOperatorConfigsItem(
  operatorName: string,
): Promise<SubOperatorConfigsItem[]> {
  ...
  if (subOperatorConfigList) {
    for (const subOperatorConfig of subOperatorConfigList.items) {
      let subOperatorConfigUrl = await k8s.getResourceUrl(
        util.ZosCloudBrokerKinds.subOperatorConfig,
        util.zosCloudBrokerGroup,
        util.subOperatorConfigApiVersion,
        subOperatorConfig.metadata.name,
      );
      subOperatorConfigItems.push(
        new SubOperatorConfigsItem(subOperatorConfig, subOperatorConfigUrl),
      );
    }
  }
