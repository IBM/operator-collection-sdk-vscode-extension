...
export class OperatorCollectionsItem extends ResourceTreeItem {
  constructor(
    ...
    super(
      operatorCollectionObj.metadata.name,
...
export async function getOperatorCollectionsItem(
  operatorName: string,
): Promise<OperatorCollectionsItem[]> {
  ...
  if (operatorCollectionList) {
    for (const operatorCollection of operatorCollectionList.items) {
      let operatorCollectionUrl = await k8s.getResourceUrl(
        util.ZosCloudBrokerKinds.operatorCollection,
        util.zosCloudBrokerGroup,
        util.operatorCollectionApiVersion,
        operatorCollection.metadata.name,
      );
      operatorCollectionItems.push(
        new OperatorCollectionsItem(operatorCollection, operatorCollectionUrl),
      );
    }
  }
